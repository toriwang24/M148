...
columns = ['visitoridlow', 'checkoutthankyouflag', 'addonsymal', 'post_evar22', 'cdedspomodel', 'evar83', 'prop29', 'visitnumber', 'visitpagenum']
dfs = []
for member in tar:
    if member.isreg():
...
### Model set up ###
opt_alg = keras.optimizers.Adam(learning_rate=0.0025)
...
#----------------------------------------------------------#
hist = model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=1000, batch_size=360)
