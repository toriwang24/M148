...
def feature_importance(df):
    ...
    n_features = df_imp.iloc[0:20, 0]

    return processed_df, n_features, target

def encode_data(processed_df, n_features, target):
    ### Subset important variables and transform data ###
    df_target = processed_df['target']
    x_train = processed_df[n_features]
    x_train = pd.concat([df_target, x_train], axis=1)
    ...
    # Target encoding
    targ_enc = TargetEncoder(handle_unknown = 'value')
    targ_enc.fit(x_train, y_train_transformed)
    x_train_transformed = targ_enc.transform(x_train)
