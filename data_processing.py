...
def read_data(data_file):
    ...
    df13 = pd.read_parquet(tn13.name)
    df14 = pd.read_parquet(tn14.name)
    df15 = pd.read_parquet(tn15.name)
    df16 = pd.read_parquet(tn16.name)

    df = pd.concat([df, df1, df4, df5, df8, df9, df12, df13, df15, df16])
...
def feature_importance(df):
    ...
    x_train, x_test, y_train, y_test = train_test_split(df_third_subset.iloc[:,:], 
                                                        ...
                                                        test_size=0.10, random_state=148)
    ...
    x_train_numerical_transformed = x_train_numerical_transformed.fillna(-1)
    x_test_numerical_transformed = pd.DataFrame(x_test_numerical_transformed)
    x_test_numerical_transformed = x_test_numerical_transformed.fillna(-1)
    ...
    n_features = df_imp.iloc[0:100, 0]
