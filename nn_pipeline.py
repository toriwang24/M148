...
top_features = top_features.iloc[0:40] # Can take anywhere from 20-40 features
print(top_features)
random_seed = 148 # Initialize random seed
...
x_train_2 = x_full_train[x_full_train['target'] == 0].sample(n=round(len(y_train_1)), random_state=random_seed)
...
x_test_2 = x_full_test[x_full_test['target'] == 0].sample(n=len(y_test_1), random_state=random_seed)
...
model = Sequential()
model.add(Dense(128, activation='sigmoid', input_dim=len(top_features))) # Sigmoid works better than ReLU and Tanh
...
ax.hist(model.predict(x_test) * 100, bins = range(0,110,10))
